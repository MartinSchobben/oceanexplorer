...
-   Users with minimal programming experience as a hybrid graphical that can be easily integrated in an R workflow (RStudio addin).
-   Users with an R programming background can easily load NOAA data into R.

I will now in reversed order explain some of the basic usage .. of all three application types.
...
The shiny app is hosted by Utrecht University and can be found here: *link to be included*. It allows exploration of the NOAA database. Data of selected point can be extracted as a csv file.
...
The last action also automatically drops us in the "locations" tab for the left most field, which allows us to select specific regions on the now displayed world map of the variable of interest. 
...
The RStudio addin has much the same functionality as the Shiny app except that it has fewer option for data entry. But this tools is great for integration in R scripts as the addin emits the code used for your data selection and filtering operation. The emitted code is actually the code used to generate the graphical output and is the backbone of the Shiny app and RStudio addin.
...
In this case we are interested in phosphate concentrations in the month December from the Agulhas Basin at a depth 0f 1000 meter below sealevel.
...
Where we can also show this on the previous plot.
